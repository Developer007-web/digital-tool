...
const CreateDesignConceptInput = z.object({
    design_type: DesignTypeEnum.describe("The category of design being requested. Use 'auto' to let the director infer it from the brief."),
    brief: z
        .string()
        .min(5)
        .describe("The client's request in natural language: business description, audience, vibe, constraints, competitors, anything relevant."),
...
const AnalyzeImageInput = z.object({
    image_url: z
        .string()
        .describe("Publicly accessible image URL or a base64 data URL (data:image/png;base64,...) of the design/logo/competitor asset to analyze."),
    analysis_goal: z
        .string()
        .optional()
...
const TOOLS = [
    {
        ...
        description: "Acts as a senior creative director to produce a full production-ready design concept: project understanding, creative direction, visual strategy, typography, color system, layout, brand psychology, production specs for Figma/Canva/Photoshop/Illustrator, AI image prompts for 5 platforms, 3 variations, and a deliverable checklist. Use this as the primary tool for any logo, brand identity, thumbnail, poster, flyer, social graphic, ad, packaging, app icon, hero section, landing page, or marketing asset request.",
        inputSchema: {
            type: "object",
            properties: {
                design_type: {
                    ...
                    description: "The category of design being requested. Use 'auto' to let the director infer it from the brief.",
                },
                brief: {
                    type: "string",
                    description: "The client's request in natural language: business description, audience, vibe, constraints, competitors, anything relevant.",
                },
    ...
    },
    {
        ...
        description: "Evaluates a list of candidate brand/product names for memorability, pronounceability, trademark-risk red flags, domain/social viability heuristics, and fit with the target industry and audience. Ranks them with reasoning.",
        inputSchema: {
            type: "object",
            properties: {
                candidate_names: {
    ...
    },
    {
        ...
        description: "Analyzes named competitors' visual/brand positioning (from names/URLs provided) and recommends how the user's brand should differentiate visually — color, typography, tone — to stand apart in the same category.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Uses a Groq vision model to analyze an uploaded image — a logo, brand asset, screenshot, or competitor packaging photo — and extracts brand elements (colors, typography style, composition, tone) or performs a design critique/teardown.",
        inputSchema: {
            type: "object",
            properties: {
                image_url: {
    ...
    },
    {
        ...
        description: "Converts a finalized visual concept description into highly detailed, platform-optimized AI image generation prompts for Flux, GPT Image, Midjourney, Ideogram, and Stable Diffusion (including negative prompts).",
        inputSchema: {
            type: "object",
            properties: {
                concept_description: {
...
// Server setup
// ---------------------------------------------------------------------------
const server = new Server({ name: "designstudio-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: TOOLS }));
function textResult(content, warning) {
    return {
        content: [
            ...(warning ? [{ type: "text", text: `⚠️ ${warning}\n\n` }] : []),
    ...
    try {
        switch (name) {
            case "create_design_concept": {
                ...
                const { text, usedGroq, warning } = await runWithFallback(task, () => buildFallbackBrief(briefText, mode));
                return textResult(text, usedGroq ? undefined : warning);
            }
            case "evaluate_brand_names": {
                const input = EvaluateNamesInput.parse(args);
                const task = `Evaluate the following candidate brand names for the "${input.industry}" industry${input.target_audience ? `, targeting: ${input.target_audience}` : ""}.
...
For each name, score (1-10) on: Memorability, Pronounceability, Visual/logo potential, Trademark-risk red flags (heuristic only, not legal advice), and Domain/social-handle viability (heuristic). Then rank all candidates from strongest to weakest with a one-line verdict for each, and declare a single winning recommendation with reasoning grounded in brand strategy, not generic praise.`;
                const { text, usedGroq, warning } = await runWithFallback(task, () => {
                    const ranked = input.candidate_names
                        .map((n, i) => `${i + 1}. **${n}** — Score: ${(7 - i * 0.3 < 0 ? 5 : 7 - i * 0.3).toFixed(1)}/10. Short, check live domain/trademark availability manually before committing.`)
                        .join("\n");
...
Be specific and decisive, not generic.`;
                ...
                return textResult(`# Image Analysis Unavailable\n\nThe vision model call failed (${result.error ?? "unknown error"}). Recommendation: verify GROQ_API_KEY is set and that GROQ_VISION_MODEL points to a currently-available vision model, then retry. As a manual fallback, describe the image's dominant colors, typography, and composition in text and pass it to create_design_concept's "brief" field — the creative-direction tool will reason over that description directly.`, "Vision analysis call failed — see message body for remediation steps.");
            }
