...
const PALETTES: Record<string, PaletteSet> = {
  food: {
    ...
    accent: ["#F2C572"],
    reasoning:
      "Warm terracotta and char-brown trigger appetite and craft/heritage cues; sage green signals freshness; gold accent reads as premium without veering luxury-cold.",
  },
  saas: {
    ...
    accent: ["#00D4B5"],
    reasoning:
      "Electric indigo signals trust + innovation typical of B2B SaaS; near-black grounds it as serious infrastructure; teal accent creates a single CTA focal color competitors in this space rarely use.",
  },
  startup: {
    ...
    accent: ["#FF4D2E"],
    reasoning:
      "Stark black/cream signals confidence and editorial credibility; a single hot accent isolates the CTA and brand mark so it survives compression on small screens.",
  },
  restaurant: {
    ...
    accent: ["#C9A227"],
    reasoning:
      "Deep forest + bone cream reads as an elevated, ingredient-led kitchen; brass-gold accent implies craftsmanship and price point without ostentation.",
  },
  luxury: {
    ...
    accent: ["#7A1F2B"],
    reasoning:
      "Black/gold is the category-default luxury code, used deliberately here, not generically; oxblood accent breaks the cliché just enough to be memorable.",
  },
  default: {
    ...
    accent: ["#F2545B"],
    reasoning:
      "High-contrast navy/white base maximizes legibility across every export size; coral accent isolates the single most important interactive element.",
  },
};

function pickPalette(brief: string): PaletteSet {
  const b = brief.toLowerCase();
  if (/(restaurant|menu|cafe|bistro)/.test(b)) return PALETTES.restaurant;
  if (/(food|kitchen|snack|beverage|drink|sauce|spice)/.test(b)) return PALETTES.food;
  if (/(saas|app|software|platform|dashboard)/.test(b)) return PALETTES.saas;
  if (/(luxury|premium|jewel|watch|couture)/.test(b)) return PALETTES.luxury;
  if (/(startup|founder|pitch|venture)/.test(b)) return PALETTES.startup;
  return PALETTES.default;
}

function detectDesignType(brief: string): string {
  const b = brief.toLowerCase();
  if (/logo/.test(b)) return "Logo / Brand Mark";
  if (/thumbnail/.test(b)) return "Video Thumbnail";
  if (/(poster)/.test(b)) return "Poster";
  if (/(flyer)/.test(b)) return "Flyer";
  if (/(packaging|package|box|label)/.test(b)) return "Product Packaging";
  if (/(app icon)/.test(b)) return "App Icon";
  if (/(hero|landing page|website)/.test(b)) return "Website Hero / Landing Page";
  if (/(social|instagram|post)/.test(b)) return "Social Media Graphic";
  if (/(ad|advertisement|campaign)/.test(b)) return "Advertisement";
  if (/(deck|presentation|slide)/.test(b)) return "Presentation Visual";
  return "Brand Identity System";
}

export function buildFallbackBrief(brief: string, mode: string): string {
